...
const sendEmail = async ({ to, subject, text, html, ...rest }) => {
  try {
    console.log('log:', process.env.NODE_MAILER_USER);
    console.log('log:', process.env.NODE_MAILER_PASSWORD);
    ...
    if (res) {
      ...
      const mail = {
        //sender access
        from: '"HNA Store" <no-reply@accounts>',
        //receiver access
        to,
        //subject
        subject,
...
const htmlSignupAccount = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        Mã xác nhận đăng ký tài khoản cho website HNA Store của anh (chị).<br />
        Cảm ơn vì đã ghé thăm HNA Store <3
    </h2>
...
const htmlResetPassword = (token) => {
  return `<div>
    ...
    <h2 style="padding: 10px 0; margin-bottom: 10px;">
        Xin chào anh (chị),<br />
        Cửa hàng HNA Store đã nhận được yêu cầu lấy lại mật khẩu từ quý khách.<br />
